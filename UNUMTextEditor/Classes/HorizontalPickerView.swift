...
/**
 Styles of HorizontalPickerView.
 - Wheel: Style with 3D appearance like UIPickerView.
 - Flat:  Flat style.
 */
public enum HorizontalPickerViewStyle {
    case wheel
    case flat
}
 ...
 */
@objc public protocol HorizontalPickerViewDataSource {
    func numberOfItemsInPickerView(_ pickerView: HorizontalPickerView) -> Int
    @objc optional func pickerView(_ pickerView: HorizontalPickerView, titleForItem item: Int) -> String
    @objc optional func pickerView(_ pickerView: HorizontalPickerView, imageForItem item: Int) -> UIImage
}
 ...
 */
@objc public protocol HorizontalPickerViewDelegate: UIScrollViewDelegate {
    @objc optional func pickerView(_ pickerView: HorizontalPickerView, didSelectItem item: Int)
    @objc optional func pickerView(_ pickerView: HorizontalPickerView, marginForItem item: Int) -> CGSize
    @objc optional func pickerView(_ pickerView: HorizontalPickerView, configureLabel label: UILabel, forItem item: Int)
}

// MARK: - Private Classes and Protocols
// MARK: HorizontalCollectionViewLayoutDelegate
/**
 Private. Used to deliver the style of the picker.
 */
private protocol HorizontalCollectionViewLayoutDelegate {
    func pickerViewStyleForCollectionViewLayout(_ layout: HorizontalCollectionViewLayout) -> HorizontalPickerViewStyle
}

// MARK: HorizontalCollectionViewCell
/**
 Private. A subclass of UICollectionViewCell used in HorizontalPickerView's collection view.
 */
private class HorizontalCollectionViewCell: UICollectionViewCell {
    var label: UILabel!
...
// MARK: HorizontalCollectionViewLayout
/**
 Private. A subclass of UICollectionViewFlowLayout used in the collection view.
 */
private class HorizontalCollectionViewLayout: UICollectionViewFlowLayout {
    var delegate: HorizontalCollectionViewLayoutDelegate!
    var width: CGFloat!
    var midX: CGFloat!
...
// MARK: HorizontalPickerViewDelegateIntercepter
/**
 Private. Used to hook UICollectionViewDelegate and throw it HorizontalPickerView,
 and if it conforms to UIScrollViewDelegate, also throw it to HorizontalPickerView's delegate.
 */
private class HorizontalPickerViewDelegateIntercepter: NSObject, UICollectionViewDelegate {
    weak var pickerView: HorizontalPickerView?
    weak var delegate: UIScrollViewDelegate?

    init(pickerView: HorizontalPickerView, delegate: UIScrollViewDelegate?) {
 ...
 */
public class HorizontalPickerView: UIView, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, HorizontalCollectionViewLayoutDelegate {

    // MARK: - Properties
    // MARK: Readwrite Properties
    /// Readwrite. Data source of picker view.
    public weak var dataSource: HorizontalPickerViewDataSource? = nil
    /// Readwrite. Delegate of picker view.
    public weak var delegate: HorizontalPickerViewDelegate? = nil {
        didSet(delegate) {
    ...
    /// Readwrite. The style of the picker view. See HorizontalPickerViewStyle.
    public var pickerViewStyle = HorizontalPickerViewStyle.wheel

    /// Readwrite. A float value which determines the perspective representation which used when using HorizontalPickerViewStyle.Wheel style.
    ...
    fileprivate var intercepter: HorizontalPickerViewDelegateIntercepter!
    /// Private. A UICollectionViewFlowLayout used in picker view's collection view.
    fileprivate var collectionViewLayout: HorizontalCollectionViewLayout {
    ...
    fileprivate func initialize() {
        ...
        self.collectionView.register(
            ...
            forCellWithReuseIdentifier: NSStringFromClass(HorizontalCollectionViewCell.self))
        self.addSubview(self.collectionView)

        self.intercepter = HorizontalPickerViewDelegateIntercepter(pickerView: self, delegate: self.delegate)
    ...
    fileprivate func selectItem(_ item: Int, animated: Bool, notifySelection: Bool) {
        ...
        if self.dataSource!.numberOfItemsInPickerView(self) <= item {
            item = self.dataSource!.numberOfItemsInPickerView(self) - 1
        }
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: NSStringFromClass(HorizontalCollectionViewCell.self), for: indexPath) as! HorizontalCollectionViewCell
    ...
    // MARK: HorizontalCollectionViewLayoutDelegate
    fileprivate func pickerViewStyleForCollectionViewLayout(_ layout: HorizontalCollectionViewLayout) -> HorizontalPickerViewStyle {
